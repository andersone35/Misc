module AStrO_r_elementEqns
	...
	subroutine r_getInstDof(instU,globU,globNds,instOri,locOri,rot,drIdrG,numNds,dofTable,dv,dv2)
		...
		integer, intent(in) :: dofTable(2,33)
		real*8, intent(in) :: globU(6,11), globNds(3,10), instOri(3,48), locOri(3,3), rot(3), drIdrG(3,16)
		real*8, intent(out) :: instU(6,11)

        real*8 :: nnInv, delRot(3), ddelRot(3)
	...
	subroutine r_getShellFrcMom(frcMom,shDef,ptT,elNum,sPt)
		...
		real*8 :: instOri(3,48), drIdrG(3,16), rot(3), ptRot(3)
		real*8 :: instU(6,11)
		real*8 :: rnNds
		integer :: i1, i2, i3
	...
	subroutine r_getElBodyForce(ndFrc,dofTable,ndDof,bdyFrc,isGrav,elNum)
		...
		real*8 :: temp(10), Tdot(10), pTemp(10), pTdot(10)
		real*8 :: disp(6,11), vel(6,11), acc(6,11), pDisp(6,11), pVel(6,11), pAcc(6,11)
		real*8 :: statInOri(3,48), statdrIdrG(3,16), statRot(3)		
	...
	subroutine r_getElCentAcc(centAcc,center,axis,angVel,elNum)
		...
		real*8 :: temp(10), Tdot(10), pTemp(10), pTdot(10)
		real*8 :: disp(6,11), vel(6,11), acc(6,11), pDisp(6,11), pVel(6,11), pAcc(6,11)
		real*8 :: statInOri(3,48), statdrIdrG(3,12), statRot(3)		
	...
	subroutine r_getElRum(Rum,dRdA,buildMat,eType,numNds,dofPerNd,numIntDof,numIntPts,dofTable,intPts,ipWt, &
		...
		real*8, intent(in) :: statInOri(3,48), statdrIdrG(3,16), statRot(3)
        real*8, intent(in) :: den, sMass(6,6), bMass(6,6)
        real*8, intent(in) :: disp(6,11), acc(6,11)
		
		real*8 :: instDofMat(6,363), instNodalAcc(6,4), iNAM(6,4), inerFrc(6), delU(6), accI
		...
		if(eType .eq. 41 .or. eType .eq. 3 .or. eType .eq. 2) then
		    instDofMat(:,:) = r_0
			call r_getInstDof(instDofMat(:,1:11),disp,globNds,statInOri,orient,statRot,statdrIdrG,numNds,dofTable,0,0)
			i1 = numNds*dofPerNd
			i3 = 11
			instNodalAcc(:,:) = r_0
			do i2 = 1, i1
			    call r_getInstDof(instDofMat(:,i3+1:i3+11),disp,globNds,statInOri,orient,statRot,statdrIdrG,numNds,dofTable,i2,0)
				i4 = dofTable(1,i2)
				i5 = dofTable(2,i2)
